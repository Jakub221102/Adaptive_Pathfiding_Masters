...
def _successors(
    ...
    for row_delta, col_delta in _MOVEMENT_DELTAS:
        ...
        if _is_valid_cell(grid_map, next_row, next_col):
            successors.append(
                TimedState(row=next_row, col=next_col, timestep=next_timestep)
            )
...
def find_path(
    ...
    while open_heap:
        ...
        for successor in _successors(grid_map, current, max_timestep):
