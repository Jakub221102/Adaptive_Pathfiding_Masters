"""Execute MAPF-7 conflict-aware priority benchmark on the stored MAPF-5A catalogue.

Open in PyCharm and press Run. No command-line arguments required.
Edit the MAPF-7 LOCAL BENCHMARK CONFIGURATION block below to change settings.
"""
...
from pathfinding.src.experiments.mapf_conflict_aware_priority_execution import (
    ...
    MAIN_CONFLICT_AWARE_CSV,
    MAIN_CONFLICT_AWARE_JSONL,
    MAIN_CONFLICT_AWARE_LOG,
...
# ============================================================
# MAPF-7 CONFLICT-AWARE PRIORITY LOCAL BENCHMARK CONFIGURATION
# ============================================================
...
# Set SMOKE_MODE=True for a tiny checkpoint/resume test (1 instance, all 3 strategies).
SMOKE_MODE = False
...
RESULTS_DIR = _REPO_ROOT / MAIN_CONFLICT_AWARE_RESULTS_DIR
RESULTS_CSV = _REPO_ROOT / MAIN_CONFLICT_AWARE_CSV
RESULTS_JSONL = _REPO_ROOT / MAIN_CONFLICT_AWARE_JSONL
LOG_FILE = _REPO_ROOT / MAIN_CONFLICT_AWARE_LOG
...
STRATEGY_LABELS: dict[ConflictAwareStrategy, str] = {
    ConflictAwareStrategy.CDF_H: "Conflict-Degree-First (high-first)",
    ConflictAwareStrategy.CDF_L: "Conflict-Degree-First (low-first)",
    ConflictAwareStrategy.SPF_CD: "Shortest-Path-First + Conflict-Degree tie-break",
}
...
def _setup_logging(log_file: Path, *, append: bool) -> logging.Logger:
    ...
    logger = logging.getLogger("mapf_conflict_aware_priority_execution")
...
def _print_startup_summary(
    ...
    *,
    ...
    _log_blank(logger)
    logger.info("MAPF-7 CONFLICT-AWARE PRIORITY BENCHMARK EXECUTION")
    _log_blank(logger)
...
def _print_completion_summary(
    ...
    _log_blank(logger)
    logger.info("=" * 72)
    logger.info("MAPF-7 CONFLICT-AWARE PRIORITY BENCHMARK EXECUTION COMPLETE")
    logger.info("=" * 72)
...
def run_conflict_aware_priority_benchmark() -> None:
    timer = ElapsedTimer()
...
def main() -> None:
    try:
        run_conflict_aware_priority_benchmark()
    except SystemExit:
        raise
    except Exception:
        traceback.print_exc()
