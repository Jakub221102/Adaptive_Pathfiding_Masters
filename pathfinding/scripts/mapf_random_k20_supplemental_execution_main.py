"""Execute supplemental Random K=20 orderings (indices 10..19) on the primary catalogue.

Open in PyCharm and press Run. No command-line arguments required.
Edit the SUPPLEMENTAL RANDOM K=20 LOCAL BENCHMARK CONFIGURATION block below.
"""
...
# ============================================================
# SUPPLEMENTAL RANDOM K=20 LOCAL BENCHMARK CONFIGURATION
# ============================================================
...
HISTORICAL_K10_JSONL = _REPO_ROOT / MAIN_RANDOM_PRIORITY_JSONL

# Set SMOKE_MODE=True for a tiny checkpoint/resume test (1 instance, 1 ordering).
SMOKE_MODE = False

ORDERING_INDEX_START = SUPPLEMENTAL_ORDERING_INDEX_START
SUPPLEMENTAL_COUNT = SUPPLEMENTAL_ORDERING_COUNT
...
SMOKE_SUPPLEMENTAL_COUNT = 1

RESUME = False
RESET_RESULTS = True
SAVE_AFTER_EACH_RUN = True
STOP_ON_ERROR = False
VERIFY_HISTORICAL_K10 = True

RESULTS_DIR = _REPO_ROOT / "pathfinding" / "results" / "mapf_random_k20_supplemental"
...
def _effective_supplemental_count() -> int:
    return SMOKE_SUPPLEMENTAL_COUNT if SMOKE_MODE else SUPPLEMENTAL_COUNT
...
def _setup_logging(log_file: Path, *, append: bool) -> logging.Logger:
    ...
    logger = logging.getLogger("mapf_random_k20_supplemental_execution")
...
def _print_startup_summary(
    ...
    *,
    ...
    ordering_index_end = ordering_index_start + supplemental_count - 1
    _log_blank(logger)
    logger.info("RANDOM K=20 SUPPLEMENTAL EXECUTION (orderings 10..19)")
    _log_blank(logger)
    logger.info(f"Manifest:\n  {manifest_path}")
    logger.info(f"Historical K=10 JSONL (read-only):\n  {historical_k10_jsonl}")
    _log_blank(logger)
    logger.info(f"Smoke mode: {smoke_mode}")
    logger.info(f"Historical K={DEFAULT_RANDOM_PRIORITY_K} ordering indices: 0..{DEFAULT_RANDOM_PRIORITY_K - 1}")
    logger.info(
        f"Supplemental ordering indices: "
        f"{ordering_index_start}..{ordering_index_end}"
    )
    logger.info(f"Supplemental orderings per instance: {supplemental_count}")
    ...
    logger.info(f"Verify historical K=10: {VERIFY_HISTORICAL_K10}")
    ...
    logger.info(
        f"    ordering {ordering_index_start} .. "
        f"{ordering_index_start + supplemental_count - 1}"
    )
    _log_blank(logger)
...
def _print_run_banner(
    ...
    *,
    ...
    relative_index = entry.ordering.ordering_index - ordering_index_start + 1
    _log_blank(logger)
    logger.info("=" * 60)
    ...
    logger.info(
        f"Ordering index: {entry.ordering.ordering_index} "
        f"(supplemental {relative_index} / {supplemental_count}, "
...
def _print_completion_summary(
    ...
    _log_blank(logger)
    logger.info("=" * 72)
    logger.info("RANDOM K=20 SUPPLEMENTAL EXECUTION COMPLETE")
    logger.info("=" * 72)
...
def run_random_k20_supplemental() -> None:
    timer = ElapsedTimer()
    ...
    if VERIFY_HISTORICAL_K10 and not HISTORICAL_K10_JSONL.is_file():
        raise FileNotFoundError(
            f"Historical K=10 checkpoint not found: {HISTORICAL_K10_JSONL}"
        )

    supplemental_count = _effective_supplemental_count()
    ...
    plan = build_supplemental_random_priority_run_plan(
        manifest,
        base_seed=BASE_SEED,
        historical_k10_jsonl_path=HISTORICAL_K10_JSONL,
        ...
        verify_historical_k10=VERIFY_HISTORICAL_K10,
    )
    total_runs = len(plan)
