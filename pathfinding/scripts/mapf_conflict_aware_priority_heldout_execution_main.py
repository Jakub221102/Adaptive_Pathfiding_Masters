"""Execute MAPF-7 conflict-aware priority benchmark on the FROZEN held-out catalogue.

Open in PyCharm and press Run. No command-line arguments required.
Edit the MAPF-7 HELD-OUT VALIDATION CONFIGURATION block below to change settings.

This runner does NOT modify the held-out manifest or primary MAPF-7 results.
"""
...
from pathfinding.src.experiments.mapf_conflict_aware_priority_execution import (
    ...
    HELD_OUT_CONFLICT_AWARE_CSV,
    HELD_OUT_CONFLICT_AWARE_JSONL,
    HELD_OUT_CONFLICT_AWARE_LOG,
    ...
    ConflictAwarePriorityRunPlanEntry,
...
# ============================================================
# MAPF-7 HELD-OUT VALIDATION CONFIGURATION
# ============================================================
...
# Set SMOKE_MODE=True for infrastructure validation only (1 instance × 3 strategies).
SMOKE_MODE = False

STRATEGIES: tuple[ConflictAwareStrategy, ...] = DEFAULT_CONFLICT_AWARE_STRATEGIES
...
RESULTS_DIR = _REPO_ROOT / HELD_OUT_CONFLICT_AWARE_RESULTS_DIR
RESULTS_CSV = _REPO_ROOT / HELD_OUT_CONFLICT_AWARE_CSV
RESULTS_JSONL = _REPO_ROOT / HELD_OUT_CONFLICT_AWARE_JSONL
LOG_FILE = _REPO_ROOT / HELD_OUT_CONFLICT_AWARE_LOG

# ============================================================
# END CONFIGURATION
# ============================================================

STRATEGY_LABELS: dict[ConflictAwareStrategy, str] = {
    ConflictAwareStrategy.CDF_H: "CDF-H (Conflict-Degree-First, high-first)",
    ConflictAwareStrategy.CDF_L: "CDF-L (Conflict-Degree-First, low-first)",
    ConflictAwareStrategy.SPF_CD: "SPF+CD (Shortest-Path-First + Conflict-Degree tie-break)",
}
...
def _setup_logging(log_file: Path, *, append: bool) -> logging.Logger:
    ...
    logger = logging.getLogger("mapf_conflict_aware_priority_heldout_execution")
...
def _count_by_outcome(
    records: Sequence[ConflictAwarePriorityRunRecord],
) -> dict[str, int]:
    counts = {
...
def _print_startup_summary(
    ...
    *,
    manifest_path: Path,
    catalogue_seed: int,
    strategies: Sequence[ConflictAwareStrategy],
    max_timestep: int,
    ...
    _log_blank(logger)
    logger.info("MAPF-7 HELD-OUT VALIDATION")
    _log_blank(logger)
    ...
    for strategy in strategies:
        logger.info(f"  - {STRATEGY_LABELS[strategy]} ({strategy.value})")
    ...
    logger.info("    CDF-H, then CDF-L, then SPF+CD")
    _log_blank(logger)
...
def _print_run_banner(
    ...
    *,
    ...
    logger.info(
        f"Strategy: {STRATEGY_LABELS[entry.strategy]} ({entry.strategy.value})"
    )
    logger.info("=" * 60)
    _log_blank(logger)
...
def _print_run_result(
    ...
    if record.original_index_order is not None:
        logger.info(f"original_index_order: {list(record.original_index_order)}")
    if record.agent_degrees is not None:
        logger.info(f"agent_degrees: {list(record.agent_degrees)}")
    if record.independent_conflict_count is not None:
        logger.info(
            f"independent_conflict_count: {record.independent_conflict_count}"
        )
    if record.success:
...
def _print_completion_summary(
    ...
    _log_blank(logger)
    logger.info("=" * 72)
    logger.info("MAPF-7 HELD-OUT VALIDATION EXECUTION COMPLETE")
    logger.info("=" * 72)
...
def _print_interrupt_summary(
    ...
    *,
    ...
    _log_blank(logger)
    logger.info("=" * 72)
    logger.info("HELD-OUT BENCHMARK INTERRUPTED BY USER")
    logger.info("=" * 72)
...
def run_held_out_conflict_aware_priority_benchmark() -> None:
    timer = ElapsedTimer()
    ...
    primary_results_jsonl = _REPO_ROOT / "pathfinding/results/mapf_conflict_aware_priority_execution/results_details.jsonl"
    primary_results_digest_before: str | None = None
    if primary_results_jsonl.is_file():
        primary_results_digest_before = _file_digest(primary_results_jsonl)

    instance_ids = _effective_instance_ids()

    checkpoint = ConflictAwarePriorityCheckpointStore(
    ...
    completed = 0
    session_records: list[ConflictAwarePriorityRunRecord] = list(
        checkpoint.records.values()
    )

    try:
        for run_number, entry in enumerate(plan, start=1):
            key = ConflictAwarePriorityRunKey(
    ...
    if primary_manifest_digest_before is not None:
        if _file_digest(primary_manifest_path) != primary_manifest_digest_before:
            raise RuntimeError("Primary manifest was modified during held-out execution")

    if _file_digest(MANIFEST_PATH) != held_out_manifest_digest_before:
        raise RuntimeError("Held-out manifest was modified during execution")

    if primary_results_digest_before is not None:
        if not primary_results_jsonl.is_file():
            raise RuntimeError("Primary MAPF-7 results file disappeared during held-out execution")
        if _file_digest(primary_results_jsonl) != primary_results_digest_before:
            raise RuntimeError("Primary MAPF-7 results were modified during held-out execution")
...
def main() -> None:
    try:
        run_held_out_conflict_aware_priority_benchmark()
    except SystemExit:
        raise
    except Exception:
        traceback.print_exc()
