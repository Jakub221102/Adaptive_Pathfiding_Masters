"""Execute MAPF-6E SPF/LPF benchmark on the stored MAPF-5A catalogue.

Open in PyCharm and press Run. No command-line arguments required.
Edit the MAPF-6E LOCAL BENCHMARK CONFIGURATION block below to change settings.
"""
...
# ============================================================
# MAPF-6E SPF/LPF LOCAL BENCHMARK CONFIGURATION
# ============================================================
...
# Set SMOKE_MODE=True for a tiny checkpoint/resume test (1 instance, SPF + LPF).
SMOKE_MODE = False

STRATEGIES: tuple[PriorityStrategy, ...] = DEFAULT_PRIORITY_STRATEGIES
...
RESULTS_DIR = _REPO_ROOT / "pathfinding" / "results" / "mapf_priority_strategy_execution"
...
STRATEGY_LABELS: dict[PriorityStrategy, str] = {
    PriorityStrategy.SPF: "Shortest-Path-First",
    PriorityStrategy.LPF: "Longest-Path-First",
}
...
def _setup_logging(log_file: Path, *, append: bool) -> logging.Logger:
    ...
    logger = logging.getLogger("mapf_priority_strategy_execution")
...
def _print_startup_summary(
    ...
    *,
    manifest_path: Path,
    strategies: Sequence[PriorityStrategy],
    max_timestep: int,
    ...
    _log_blank(logger)
    logger.info("MAPF-6E SPF/LPF BENCHMARK EXECUTION")
    _log_blank(logger)
...
def _print_run_banner(
    ...
    *,
    ...
    logger.info(f"Strategy: {STRATEGY_LABELS[entry.strategy]} ({entry.strategy.value})")
    logger.info("=" * 60)
    _log_blank(logger)
...
def _print_completion_summary(
    ...
    _log_blank(logger)
    logger.info("=" * 72)
    logger.info("MAPF-6E SPF/LPF BENCHMARK EXECUTION COMPLETE")
    logger.info("=" * 72)
...
def run_priority_strategy_benchmark() -> None:
    timer = ElapsedTimer()
    ...
    completed = 0
    session_records: list[PriorityStrategyRunRecord] = list(checkpoint.records.values())
...
def main() -> None:
    try:
        run_priority_strategy_benchmark()
    except SystemExit:
        raise
    except Exception:
        traceback.print_exc()
